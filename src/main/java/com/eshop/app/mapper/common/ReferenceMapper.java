...
public class ReferenceMapper {
    ...
    public <T> T resolve(Long id, @TargetType Class<T> entityClass) {
        ...
        if (entityClass == User.class) return entityClass.cast(resolveUser(id));
        if (entityClass == Category.class) return entityClass.cast(resolveCategory(id));
        if (entityClass == Brand.class) return entityClass.cast(resolveBrand(id));
        if (entityClass == Shop.class) return entityClass.cast(resolveShop(id));
        if (entityClass == Product.class) return entityClass.cast(resolveProduct(id));
        if (entityClass == Tag.class) return entityClass.cast(resolveTag(id));
    ...
    @Named("idToShop")
    public Shop resolveShop(Long id) {
        if (id == null) return null;
    ...
    @Named("userToId")
    public Long toUserId(User user) { return user != null ? user.getId() : null; }

    @Named("categoryToId")
    public Long toCategoryId(Category category) { return category != null ? category.getId() : null; }

    @Named("brandToId")
    public Long toBrandId(Brand brand) { return brand != null ? brand.getId() : null; }

    @Named("shopToId")
    public Long toShopId(Shop shop) { return shop != null ? shop.getId() : null; }
