...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT o FROM Order o JOIN o.items oi WHERE oi.product.shop.id = :shopId")
    ...
    long countByDeliveryAgentIdAndOrderStatusAndCreatedAtAfter(Long agentId, Order.OrderStatus orderStatus, LocalDateTime createdAt);
    long countByDeliveryAgentId(Long agentId);
    long countByOrderStatus(Order.OrderStatus orderStatus);
    
    @Query("SELECT COALESCE(SUM(o.totalAmount), 0) FROM Order o JOIN o.items oi WHERE oi.product.shop.seller.id = :sellerId AND o.createdAt BETWEEN :startDate AND :endDate")
    BigDecimal sumRevenueBySellerIdBetweenDates(@Param("sellerId") Long sellerId, @Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
    
    @Query("SELECT COUNT(o) FROM Order o JOIN o.items oi WHERE oi.product.shop.seller.id = :sellerId AND o.orderStatus = :status")
    long countByShopSellerIdAndOrderStatus(@Param("sellerId") Long sellerId, @Param("status") Order.OrderStatus status);
    
    @Query("SELECT o FROM Order o JOIN o.items oi WHERE oi.product.shop.seller.id = :sellerId ORDER BY o.createdAt DESC")
