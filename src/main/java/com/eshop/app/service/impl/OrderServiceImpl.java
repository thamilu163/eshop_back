...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional(readOnly = true)
    public PageResponse<OrderResponse> getOrdersByShop(Long shopId, Pageable pageable) {
    ...
    private OrderResponse processCheckout(Cart cart, CheckoutRequest request, Long userId) {
        ...
        // Calculate totals (using basic calculation since CheckoutRequest doesn't have tax/shipping fields)
    ...
    public BigDecimal getTotalRevenueBySellerId(Long sellerId) {
        return orderRepository.sumRevenueBySellerIdBetweenDates(sellerId, java.time.LocalDateTime.MIN, java.time.LocalDateTime.now());
    }
    
    @Override
    @Transactional(readOnly = true)
    public long getNewOrderCountBySellerId(Long sellerId) {
        return orderRepository.countByShopSellerIdAndOrderStatus(sellerId, Order.OrderStatus.PLACED);
    }
    
    @Override
    @Transactional(readOnly = true)
    public long getProcessingOrderCountBySellerId(Long sellerId) {
        return orderRepository.countByShopSellerIdAndOrderStatus(sellerId, Order.OrderStatus.CONFIRMED);
    }
    
    @Override
    @Transactional(readOnly = true)
    public long getShippedOrderCountBySellerId(Long sellerId) {
        return orderRepository.countByShopSellerIdAndOrderStatus(sellerId, Order.OrderStatus.SHIPPED);
    }
    
    @Override
    @Transactional(readOnly = true)
    public long getCompletedOrderCountBySellerId(Long sellerId) {
        return orderRepository.countByShopSellerIdAndOrderStatus(sellerId, Order.OrderStatus.DELIVERED);
    }
    
    @Override
    @Transactional(readOnly = true)
    public java.util.List<java.util.Map<String, Object>> getRecentOrdersBySellerId(Long sellerId, int limit) {
        return orderRepository.findRecentOrdersBySellerId(sellerId, org.springframework.data.domain.PageRequest.of(0, limit))
                .stream()
                .map(o -> {
    ...
    public long getCompletedDeliveriesTodayByAgentId(Long agentId) {
        java.time.LocalDateTime startOfDay = java.time.LocalDateTime.now().withHour(0).withMinute(0).withSecond(0);
        return orderRepository.countByDeliveryAgentIdAndOrderStatusAndCreatedAtAfter(agentId, Order.OrderStatus.DELIVERED, startOfDay);
    }
    ...
    public long getCompletedDeliveriesThisWeekByAgentId(Long agentId) {
        java.time.LocalDateTime startOfWeek = java.time.LocalDateTime.now().minusWeeks(1);
        return orderRepository.countByDeliveryAgentIdAndOrderStatusAndCreatedAtAfter(agentId, Order.OrderStatus.DELIVERED, startOfWeek);
    }
    ...
    public long getCompletedDeliveriesThisMonthByAgentId(Long agentId) {
        java.time.LocalDateTime startOfMonth = java.time.LocalDateTime.now().withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0);
        return orderRepository.countByDeliveryAgentIdAndOrderStatusAndCreatedAtAfter(agentId, Order.OrderStatus.DELIVERED, startOfMonth);
    }
    ...
    public java.util.List<java.util.Map<String, Object>> getRecentOrdersByCustomerId(Long customerId, int limit) {
        Page<Order> orders = orderRepository.findByCustomerId(customerId, org.springframework.data.domain.PageRequest.of(0, limit));
